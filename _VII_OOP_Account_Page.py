...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    def populate_user_info(username):  # For needed input once logIn
        ...
        if user:
            info = [First_Name, Middle_Name, Last_Name, Suffix, Department, Designation, Username, Password,
                    Employee_Number]
            for i in range(9):
                info[i].delete(0, 'end')
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    def updateData():  # For Update Button
        ...
        cursor.execute("SELECT * FROM III_employee_user_account_information_tbl WHERE Employee_Number=?",
                       (employee_number,))
        ...
        if record_main and record_personal and record_account:
            ...
            query4 = """UPDATE I_employee_personal_information_tbl SET First_Name=?, Middle_Name=?, Last_Name=?, Suffix=?, Department=?, Designation=? WHERE Employee_Number=?"""
